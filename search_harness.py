...
def gen_rule(rng):
    ...
    lhs = [tuple(str(t) for t in rng.choice(pool, 2, replace=True))
    ...
    rhs = [tuple(str(t) for t in rng.choice(tokens, 2, replace=True))
...
def score_rule(lhs, rhs, rng, max_edges=1500):
    try:
        edges, nid = rewrite(lhs, rhs, SEED_EDGES, max_edges=max_edges,
                             max_gen=300, seed=1)
        ...
        return {"valid": True, "N": n, "ball": db, "spec": ds, "eff": de,
                "spread": spread, "dim": dim_mean, "angle_rho": arho,
                "score": score}
...
def search(n_rules=40, seed=0):
    ...
    print(f"  {'score':>6} {'angle_rho':>9} {'spread':>7} {'dim':>5} "
          f"{'N':>5}   rule (lhs -> rhs)")
    for m in results[:10]:
