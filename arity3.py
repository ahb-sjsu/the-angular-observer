...
def gen_rule3(rng):
    ...
    lhs = [tuple(str(t) for t in rng.choice(pool, 3, replace=True))
    ...
    rhs = [tuple(str(t) for t in rng.choice(tokens, 3, replace=True))
...
def score_rule3(lhs, rhs, rng, max_edges=1500):
    try:
        tris, nid = rewrite(lhs, rhs, SEED3, max_edges=max_edges,
                            max_gen=300, seed=1)
        ...
        gated = 1.5 <= dim <= 4.5
        return {"valid": True, "N": n, "tris": len(tris), "ball": db, "spec": ds,
                "eff": de, "spread": spread, "dim": dim, "angle_rho": arho,
...
if __name__ == "__main__":
    ...
    print(f"  N={m['N']} tris={m.get('tris')} | ball={m['ball']:.2f} "
          f"spectral={m['spec']:.2f} eff_rank={m['eff']:.2f} | "
          f"dim={m['dim']:.2f} spread={m['spread']:.2f} angle_rho={m['angle_rho']:.2f}")
    print(f"  (expect dim ~2 for a refined triangular surface)\n")
    ...
    for m in res[:8]:
        print(f"  {m['score']:6.3f} {m['angle_rho']:6.3f} {m['spread']:7.2f} "
