...
RULES = {
    "R_SR (SetReplace canonical)": dict(
        ...
        claim="~2-2.5D (no explicit claim)",
        src="[R_SR] SetReplace README (verified)",
    ),
    "R_2D (announcement)": dict(
        ...
        claim="2D",
        src="[R_2D] Wolfram Apr-2020 (labels unverified)",
    ),
    "R_3D (announcement)": dict(
        ...
        seeds={"collapse": S_COLLAPSE, "tet": S_TET},
        claim="3D",
        src="[R_3D] Wolfram Apr-2020 (labels unverified)",
    ),
    "R_frac (Sierpinski)": dict(
        ...
        claim="~1.58D",
        src="[R_frac] Wolfram Apr-2020 (labels unverified)",
    ),
}
...
if __name__ == "__main__":
    ...
    print(
        "controls (rung0): 2-torus ball1.90/spec1.95/eff2.11 spread~0.09 | "
        "3-torus ball2.73/spec2.95/eff3.15\n"
    )
    print(
        f"{'rule':30s} {'seed':9s} {'Nful':>6} {'N':>5} {'tris':>5} {'deg':>5} "
    ...
    for name, R in RULES.items():
        for sname, seed in R["seeds"].items():
            ...
            if not m["ok"]:
                print(
                    f"{name:30s} {sname:9s} {'':>6} {m['N']:>5} {m['tris']:>5}  "
                    f"(stalled, <200 nodes)"
                )
                sys.stdout.flush()
                continue
            print(
                f"{name:30s} {sname:9s} {m['N_full']:>6} {m['N']:>5} {m['tris']:>5} "
