...
LIB = {
    "R_2D  (borrowed)": dict(lhs=[("1","2","2"),("3","1","4")],
        rhs=[("2","5","2"),("2","3","5"),("4","5","5")], seed=S_COLLAPSE),
    "R_frac(borrowed)": dict(lhs=[("1","2","3")],
        rhs=[("1","4","6"),("2","5","4"),("3","6","5")], seed=[(0,1,2)]),
    "evolved-2D (#2)": dict(lhs=[("a","b","c")],
        rhs=[("b","x","c"),("x","x","b"),("c","a","y")], seed=S_TET),
    "R_3D  (borrowed)": dict(lhs=[("1","1","2"),("3","4","1")],
        rhs=[("4","4","3"),("5","4","5"),("5","2","1")], seed=S_COLLAPSE),
    "R_SR  (borrowed)": dict(lhs=[("v1","v2","v3"),("v2","v4","v5")],
...
def crop(A, target=N_CAP):
    ...
    if n <= target:
        return A
    d = shortest_path(A, unweighted=True, indices=[np.random.default_rng(0).integers(n)])[0]
...
def _rho(A, w, V, modes, rng, angle=True, n_anchor=150):
    ...
    iu = np.triu_indices(len(anc), 1); g = D[iu]
...
def euclid_rho(A, rng, n_anchor=150):
    ...
    n = len(anc); J = np.eye(n) - np.ones((n, n)) / n
    B = -0.5 * J @ (D ** 2) @ J
...
def measure(name, R):
    rows = []
    for s in SEEDS:
        tris, nid = rewrite(R["lhs"], R["rhs"], R["seed"], max_edges=3000,
                            max_gen=500, seed=s)
        ...
        spread = np.std([dim_ball_growth(A), dim_spectral(w), dim_effective_rank(A, w, V)])
        low = np.arange(1, 1 + M)
        rand = rng.choice(np.arange(1, len(w)), size=M, replace=False)
        rows.append(dict(dim=dim, spread=spread,
...
if __name__ == "__main__":
    ...
    print(f"{'rule':18s} {'n':>2} {'dim':>10} {'spread':>7} {'angle_rho':>16} "
          f"{'random':>7} {'euclid':>7}")
    allpts = []
    for name, R in LIB.items():
        rows = measure(name, R)
        if not rows:
            print(f"{name:18s}  (stalled)"); continue
        d = np.array([r["dim"] for r in rows]); a = np.array([r["angle"] for r in rows])
        rr = np.array([r["random"] for r in rows]); e = np.array([r["euclid"] for r in rows])
        sp = np.array([r["spread"] for r in rows])
        for di, ai in zip(d, a):
            allpts.append((di, ai))
        print(f"{name:18s} {len(rows):2d} {d.mean():5.2f}+/-{d.std():4.2f} "
    ...
    if len(pts) >= 3:
        ...
        print(f"(B) angle_rho vs emergent dim (n={len(pts)} manifolds, d~"
              f"{pts[:,0].min():.1f}..{pts[:,0].max():.1f}): "
              f"slope={slope:+.3f}/dim, Spearman={r:+.3f}")
        print(f"    => observer fidelity {'FALLS' if slope<0 else 'RISES'} "
