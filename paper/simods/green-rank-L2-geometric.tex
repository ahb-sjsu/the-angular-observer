...
\title{A geometric bound on the $L^2$ non-radiality:\\
the shell-variance of the heat kernel}
...
\section{Imported facts and the frontier}
\noindent\textbf{Imported.} (1) Weyl heat-trace asymptotics $Z(s)\sim(4\pi s)^{-d/2}\vol$
as $s\to0$ (Minakshisundaram--Pleijel). (2) Spectral-gap lower bound $\mu_1\ge\mu_1(K,D)$
under $\Ric\ge-(d-1)K$, $\diam\le D$ (Li--Yau; Zhong--Yang for the $K=0$ sharp form). (3) The
MP short-time expansion with $u_0=\det(d\exp)^{-1/2}=1+\tfrac1{12}\Ric(\dot\gamma,\dot\gamma)r^2
+O(r^3)$. (4) $C^1$-in-$C^2$ continuity of the heat semigroup in the metric on compact
time-intervals (Kato/parabolic perturbation). (5) The distributional master inequality and
the parametrix lemma (companion note ``green-rank-positivity''). (6) The Duhamel
heat-kernel perturbation estimate $\lVert p_t^g-p_t^{g_0}\rVert_{L^2}\le C\lVert
g-g_0\rVert_{C^1}(Z^{g_0}(ct)-1)^{1/2}$, from $p_t^g-p_t^{g_0}=-\int_0^t p^g_{t-s}
(\Delta_g-\Delta_{g_0})p^{g_0}_s\,ds$ and the $L^2\!\to\!L^2$ bounds on the semigroup
(standard parabolic perturbation).

\medskip
\noindent\textbf{What is now proved.} The $L^2$ non-radiality is the shell-variance of the
Green kernel, equals the integrated heat-kernel shell-variance, \emph{vanishes on two-point
homogeneous spaces}, is finite for $d\le3$ (envelope integrable exactly there), has an
explicit spectral-gap tail, and is $O(\eta^2)$ within a $C^2$-neighborhood of any symmetric
metric---so $\rhoG\ge1-C(g_0)\eta^{2/3}\to1$, a quantitative openness (the earlier openness
theorem was purely qualitative). The empirical
plateau ($\rhoG\gtrsim0.79$ across the hunt families, \texttt{experiments/green-rank}) is the
statement that $\mathcal A$ and $V_{\min}$ stay bounded even for strong bottlenecks.

\medskip
\noindent\textbf{Frontier.} Making the anisotropy amplitude $\mathcal A$ explicit in
$(K,D,i_0,\sup\lVert\nabla\Ric\rVert)$---the leading-order estimate of Lemma~\ref{lem:head}
promoted to a two-sided bound uniform in $t\le t_0$---would upgrade Theorem~\ref{thm:main}
from a named-$\mathcal A$ class to a closed curvature/diameter theorem. This is the one
remaining non-explicit constant; everything else in the chain is explicit.
