...
truncated radius the algorithm computes, and borderline at $d=2$. Empirically the
angular representation is substrate-robust---across random-geometric lattices,
manifold samples, and hypergraph-rewriting graphs---and fails where geometry
does; its fidelity shows an exploratory decreasing trend with dimension.
Uniformity of the metric constants in the mode-count is the central open problem.
\end{abstract}
...
KV-cache quantization splits key vectors the same way \cite{polarquant2025}, and
the recurrence is evidence for the \emph{decomposition}, not for always discarding
the radius. Our hypotheses state the boundary exactly: discarding the magnitude is
the right quotient precisely when the discarded radial variable is nuisance for the
metric being preserved---the tangential-noncollapse condition (A2) of
Theorem~\ref{thm:transfer}. For graph geodesics the radius is degree/density noise
(Corollary~\ref{thm:radial}) and the quotient is justified.
The empirical
core is that the basis is \emph{substrate-robust}: it preserved geodesics
on every graph family we tested that carries genuine low-dimensional Riemannian
geometry, independent of how it was generated.

\textbf{Contributions:} (1) the \textbf{Keep-the-Angle Principle}
(\S\ref{sec:thm})---a Radial-Degeneracy \emph{corollary} (for the full
...
Angular-Preservation \emph{conjecture} now with first metric-distortion
...
(\S\ref{sec:results}), a weighting ablation (\S\ref{sec:ablation}), and a
bake-off against named spectral distances in which the angle beats every purely
spectral competitor (\S\ref{sec:bakeoff}); (3)
substrate robustness across five substrate families with a two-factor small-world
screen (\S\ref{sec:univ}); (4) an exploratory fidelity-vs-dimension trend
(\S\ref{sec:law}); and (5) two honest negatives (\S\ref{sec:neg}). We
...
Claim & Status & Where \\
\midrule
Radial degeneracy, full commute embedding & proved (vL--R--H cond.\ + bounded degree) & Cor.~\ref{thm:radial} \\
Truncated normalized radius tracks degree & empirical & \S\ref{sec:verify} \\
...
Graph transfer at fixed $m$ & proved, conditional on (H1)--(H2) & Thm.~\ref{thm:cond} \\
Global form & conditional on (H3) & Cor.~\ref{cor:global} \\
Flat torus, $m=4$ & proved, unconditional & Cor.~\ref{cor:torus} \\
Distortion $\Rightarrow$ rank & proved & Prop.~\ref{prop:rank} \\
Heat filter, uniform in $m$ & proved (continuum) & Thm.~\ref{thm:heat} \\
Commute filter, fixed-scale uniform in $m$ & refuted (exact on $\mathbb S^1$) & Prop.~\ref{prop:weyl} \\
Plain-filter rank collapse on $\mathbb S^1$ & proved & Thm.~\ref{thm:s1collapse} \\
Angular preservation (rank form) & conjecture, empirically supported & Conj.~\ref{conj:angle} \\
Uniform truncated-Jacobian bound & open problem & Conj.~\ref{conj:sigmamin} \\
\bottomrule
\end{tabular}
\end{table}
...
for the Coifman--Lafon $\alpha{=}1$ angle (\S\ref{sec:density}); empirically
$\rho_0\approx0.9$.
\end{conjecture}

We reserve ``rank-faithful'' for Spearman $\ge\rho_0>0$ over the stated class; the
diagnostic reading---using the $\omega$-screen and dimension spread to decide
whether a \emph{given} graph lies in the class---is a separate corollary-in-use,
kept out of the conjecture to avoid circularity. ``Uniformly'' means a lower bound
over the admissible family, not over all integers $m$: too few modes need not
embed $M$, a truncation inside a multiplet is basis-dependent, and $m$ may not
outrun the sample.
...
\paragraph{Metric evidence on the torus.} The metric form is no longer
...
\paragraph{The uniform-in-$m$ frontier, precisely located.} The metric form
of Conjecture~\ref{conj:angle} rests on a single named hypothesis, which we
isolate rather than assume away.
...
\noindent What is \emph{known} is weaker: B\'erard--Besson--Gallot
\cite{berard1994} and the quantitative truncated-eigenmap embeddings of
\cite{bates2014,portegies2016} give, for each fixed $m$ past an immersion
threshold, \emph{some} constant $L_0(m)$; its uniformity in $m$ is not
established, and there is structural reason to doubt the strong form---as $m$
grows the added eigenfunctions oscillate on wavelength $\mu_m^{-1/2}$, and holding
$L_0$ fixed asks that no such mode ever create a near-fold, the kind of
degeneration symmetry or a thin neck can force. Two endpoints bound the question:
\begin{itemize}
\item \emph{Satisfiable.} On the flat torus (H1)-uniform holds \textbf{globally
and unconditionally}, through the Clifford embedding into $\mathbb R^4$:
Corollary~\ref{cor:torus} is the existence proof that the hypothesis is not
vacuous.
\item \emph{Filter-specific.} For the heat/diffusion weighting the continuum
uniform-in-truncation bi-Lipschitz statement is a \emph{theorem}
(Theorem~\ref{thm:heat})---exactly the continuum content (H1)-uniform asserts.
For the commute weighting the fixed-scale form is \emph{false}
(Proposition~\ref{prop:weyl}: angular speed grows like $\sqrt\Lambda$, exactly on
$S^1$), so (H1)-uniform, if it holds for commute, is necessarily
scale-dependent. Uniformity is therefore a property of the \emph{filter}, not a
generic fact.
\end{itemize}
The empirical flatness in $m$ (\S\ref{sec:verify}) supports the \emph{rank} form;
the metric form reduces to one clean question in spectral geometry, which we state
as the sharp target.

\begin{conjecture}[Uniform truncated-Jacobian lower bound]\label{conj:sigmamin}
There is a manifold class and a gap-respecting mode schedule $m=m(n)\to\infty$
(each $m$ multiplet-closed at a spectral gap) along which the smallest singular
value of the truncated-eigenmap differential is bounded below uniformly,
\[
\inf_{m}\ \inf_{x\in M}\ \sigma_{\min}\!\big(dF_m(x)\big)\ \ge\ c>0,
\]
with $c$ depending on $M$ but not on $m$. Equivalently, \textup{(H1)-uniform}
holds along the schedule; by Theorem~\ref{thm:cond} and Lemma~\ref{lem:transfer}
this closes the metric form of Conjecture~\ref{conj:angle}.
\end{conjecture}

\noindent A proof---or a counterexample forced by symmetry---is a self-contained
problem we do not attempt here. None of the paper's proved results depends on its
resolution: the exact identity (Proposition~\ref{prop:angdist}), the transfer
theorem (Theorem~\ref{thm:transfer}), the conditional graph theorem
(Theorem~\ref{thm:cond}, unconditional in $m$ at each fixed $m$), the
unconditional torus corollary (Corollary~\ref{cor:torus}), and the filter
dichotomy (Theorem~\ref{thm:heat}, Propositions~\ref{prop:weyl} and
\ref{prop:plain} with Theorem~\ref{thm:s1collapse}) all stand independently.
...
The measured graph-level $\kappa\approx2.6$ (at $m{=}10$, inside the
...
mode-budget phenomenon of \S\ref{sec:continuum} handed a mechanism. The
...
crude metric orders coincide; Theorem~\ref{thm:s1collapse} makes the plain-side
collapse exact on the circle, where the plain ranking has vanishing Spearman
\emph{and} Kendall correlation with geodesic distance while the commute
(Green-type) correction stays monotone. Together, Proposition~\ref{prop:weyl}
explains the budget phenomenon and Proposition~\ref{prop:plain} with
Theorem~\ref{thm:s1collapse} the weighting ablation: the theory now covers both
empirical laws of \S\ref{sec:ablation}
...
proven}. Its conditional local form is now a theorem with a complete proof
...
is that Corollary~\ref{thm:radial} explains why the full, unnormalized radius
\emph{dies} for $d\ge3$ ($d=2$ borderline, \S\ref{sec:verify}), not by itself why
the angle \emph{lives}---indeed the angular fidelity
...
eigenmap-embedding mechanism, not degeneracy alone. That mechanism is
...
Conjecture~\ref{conj:angle}, and this paper's central open problem, stated sharply
as the truncated-Jacobian lower bound of Conjecture~\ref{conj:sigmamin}. Together
these are the \textbf{Keep-the-Angle Principle}: keep $\hat u_i$, discard $r_i$
---and, under strongly non-uniform sampling, keep the $\alpha{=}1$
...
\paragraph{Status of the radial claim.} (i)~\emph{Proved} ($d\ge3$, full
embedding): Corollary~\ref{thm:radial}. (ii)~\emph{Empirical} (truncated, all
substrates): $\rho(r_i,1/\sqrt{k_i})\in[0.79,0.87]$, versus $0.98$ for the full
unnormalized radius (\S\ref{sec:verify})---the gap between (i) and (ii) is exactly
the truncation, and no theorem here bridges it. (iii)~\emph{Borderline} ($d=2$):
logarithmic resistance growth, treated empirically throughout; $d=1$: no
degeneracy, as the dimension gating predicts.

\section{Methods}\label{sec:methods}
\begin{table}[h]\footnotesize\centering
\caption{Protocol by section. $n$: nodes; $m$: retained modes; $A$: anchors;
$D$: independent graph draws reported (mean$\pm$sd when $D>1$; single-draw rows
marked $\dagger$ per the headline-uncertainty policy below). \emph{Draw counts
marked} \textit{FILL} \emph{are to be read from the run ledger before submission.}}
\label{tab:protocol}
\begin{tabular}{@{}lllllll@{}}
\toprule
Section & substrate(s) & graph & $n$ & $m$ & $A$ & $D$ \\
\midrule
\S\ref{sec:results} core & 2-torus & kNN & 2000 & 5--80 & 150 & \textit{FILL} \\
\S\ref{sec:univ} & six substrates & as listed & 2000 & 20 & 150 & \textit{FILL} \\
\S\ref{sec:law} & $5$ rules $\times$ $4$ seeds & rewriting & \textit{FILL} & 20 & 150 & 4 seeds/rule \\
\S\ref{sec:verify} & 2-torus & kNN & 2000--4000 & 5--80 & 200 & $5$ ($m{=}10$) \\
\S\ref{sec:ablation} & 2-, 3-torus & kNN & 2000/3000 & 10--40 & 150 & \textit{FILL} \\
\S\ref{sec:bakeoff} & 2-, 3-torus & kNN & 2000/3000 & 20 & 250 & \textit{FILL} \\
\S\ref{sec:density} & non-uniform torus & kNN & 2000 & 20 & 150 & \textit{FILL} \\
\S\ref{sec:continuum} & RGG vs.\ emergent & $\varepsilon$/rewriting & $10^3$--$5\times10^4$ & 20 / scaled & 150 & \textit{FILL} \\
\bottomrule
\end{tabular}
\end{table}
...
swiss-roll $2$; king-lattice $2$). On the controls the estimators read low
($d_G<d_M$ by roughly $5$--$8\%$ at $n{=}2000$, a known finite-sample curvature
bias; $d_G\approx\textit{FILL}$ on the 2-torus), so all dimension-indexed claims
use $d_G$ consistently: estimator bias shifts the axis, not the ordering of the
\S\ref{sec:law} trend.
...
table of \S\ref{sec:univ} reports it at $\dim{=}2$, column Isomap$_{d=2}$). \textbf{Uncertainty.}
...
geodesics; a random-mode basis of equal size does not. The random curve is
...
\subsection{Robustness across substrates}\label{sec:univ}
\begin{center}
\begin{tabular}{lcccc}
\toprule
...
Lorentzian ($\rho=0.57$, a degradation not a failure) or destroyed. $^{*}$The small-world substrate is a heavily rewired $2$D king-lattice; the
...
Two caveats keep the robustness claim honest. First, on the two flat
...
unlike MDS, is uniform in $m$ and $n$ (\S\ref{sec:verify}). That $0.67$ is
...
two solve different problems. Robustness here means the angular basis never
\emph{loses} the geometry on a genuine low-$d$ Riemannian substrate---not that it
dominates every baseline on every substrate; we accordingly avoid
...
from points sampled on a continuous manifold. No sequential or temporal
structure enters the graph construction, so we make no sequential-data claim.

\paragraph{A two-factor small-world screen.} To make ``destroyed'' precise
...
\begin{tabular}{lcccc}
\toprule
substrate & angle-$\rho$ & $\omega$ & dim-spread & verdict \\
\midrule
...
The screen robustly separates the genuine manifolds ($\omega\le-0.50$) from
...
instrument---the mechanism behind the dimension-trend outlier below.

\subsection{An exploratory dimension--fidelity trend}\label{sec:law}
With only five independent rule families, no ``law'' can responsibly be
claimed; what the library shows is an \emph{exploratory trend}, reported at its
genuine unit. Table~\ref{tab:rulelevel} gives the five rule-level summaries (mean
over the four seeds within each rule; seeds within a rule are not independent
draws): angular fidelity falls with measured intrinsic dimension across the five
families. A linear fit to the twenty (rule$\times$seed) points---slope $-0.157$,
rule-clustered $95\%$ CI $[-0.343,-0.094]$ versus the naive point bootstrap
$[-0.185,-0.132]$ that wrongly treats the points as exchangeable;
errors-in-variables correction to $\approx-0.17$---is shown as a descriptive
trend line only; with five clusters the bootstrap is itself at the edge of
validity (\S\ref{sec:methods}), and we draw no inferential conclusion beyond sign
and rough magnitude. Proposition~\ref{prop:rank}'s exponential worst-case ceiling
is \emph{consistent with} a dimension-driven decay and shows the measured
(roughly linear) trend sits far above the distortion-certified floor; it does not
derive the trend.

\begin{table}[h]\small\centering
\caption{The five rule families at their genuine unit. Mean over four
rewrite-seed graph draws of each rule; ranges are seed min--max. The four draws
share a rule (correlated), so the rule---not the seed---is the independent unit
of the \S\ref{sec:law} analysis.}
\label{tab:rulelevel}
\begin{tabular}{@{}lccl@{}}
\toprule
Rule family & $\bar d$ (range) & angle-$\rho$ (range) & notes \\
\midrule
$R_{2\mathrm{D}}$      & $1.06$ ($1.03$--$1.07$) & $0.935$ ($0.930$--$0.941$) & quasi-1D reference \\
$R_{\mathrm{frac}}$    & $1.38$ ($1.36$--$1.39$) & $0.875$ ($0.859$--$0.890$) & \\
evolved-2D             & $1.88$ ($1.68$--$2.00$) & $0.722$ ($0.387$--$0.885$) & evolutionary search (incl.\ contaminated seed) \\
$R_{3\mathrm{D}}$      & $2.07$ ($2.05$--$2.09$) & $0.822$ ($0.816$--$0.836$) & 3D-claim rule; tightest seed spread (near-confluent) \\
$R_{\mathrm{SReach}}$  & $3.42$ ($3.32$--$3.62$) & $0.564$ ($0.461$--$0.637$) & highest $d$; lowest $\rho$ \\
\bottomrule
\end{tabular}
\end{table}

Across 20 manifolds ($5$ rules $\times\,4$ seeds; the four seeds within a
rule are not independent draws), $d\approx 1$--$3.6$,
\[
\text{angle-}\rho \;\approx\; 1.09 - 0.157\,d, \qquad
\text{slope } -0.157\ (\text{rule-clustered SE }0.061),\quad R^2 = 0.64,
\quad d\in[1.0,3.6].
\]
(Spearman $-0.881$ between $\rho$ and $d$ is a distribution-free monotonicity
...
substrate roughness. A bounded model fits essentially as well and respects
...
dimension-gated degeneracy of \S\ref{sec:thm} is the mechanism---and
...
angle sits far above its distortion-only floor. Neither mechanism, however,
\emph{derives} the fitted coefficient $-0.157$: the dimension gating explains why
fidelity falls with $d$, and Proposition~\ref{prop:rank} bounds how fast it
\emph{could}, but the observed linear magnitude is an empirical summary of these
five families, not a predicted constant.
...
the $20$ library manifolds (five Wolfram-model rules $\times$ four seeds,
...
The ablation isolates the \emph{weight}; a referee rightly asks how the
...
The angle beats every purely spectral competitor, and the margin widens in
...
\subsection{Radius is density, angle is geometry: a non-uniform torus}\label{sec:density}
The decomposition ``degree/density in the radius, geometry in the angle''
...
\subsection{Stability across substrate evolution}\label{sec:stability}
Robustness across substrate \emph{types} (\S\ref{sec:results}) is a
...
\]
and this replicates independently in both arities (arity-2 $-0.028$/dim,
...
\subsection{Continuum survival, and a dissociation}\label{sec:continuum}
Every result so far is at finite $N$ ($\approx2$--$4$k nodes, capped by
...
\end{aligned}
\]
with $d{=}2$ likewise monotone ($+0.90$, $0.916\!\to\!0.929$): refining the
...
The dissociation is with \emph{emergent} substrates. The
...
One qualification is essential, and it deepens rather than weakens the
reading: the dissociation is stated at a \emph{fixed} mode budget ($m=10$
...
regime, not an absolute substrate invariant: a manifold keeps its geodesic
geometry in a fixed low-mode band (refining sharpens those modes), whereas the
emergent substrate spreads geometry across scales (a fixed budget resolves an
ever-smaller fraction as it grows). ``Clean geometry'' is thus a relation between
mode-budget resolution and substrate scale---the sharpest form of the
...
independent signal is partial-$\rho=-0.14$. This \emph{bounds the physical
...
row-normalization a geodesic justification complementary to the
...
compression \cite{polarquant2025} (with the scope boundary
...
identically on hypergraph-rewriting and manifold-sampled substrates is
evidence that scale-invariant angular coarse-graining is a general
...
\paragraph{A boundary from compression.} The result is not a general theorem that
angular coordinates always carry the semantics: for attention \emph{keys}, whose
geometry $QK^{\top}$ depends on per-channel scale, per-vector angular quantization
can score well on reconstruction yet destroy generation---scale is signal there,
not nuisance. This is not a counterexample but the sharp scope condition:
row-normalization is justified precisely when the discarded radial variable is
nuisance for the metric being preserved---condition (A2) of
Theorem~\ref{thm:transfer}. Keep the geometry that carries the task---sometimes the
angle, sometimes angle plus norm, sometimes the per-channel scale.

\textbf{A physical interpretation, offered modestly.} In the observer-relative
reading of emergent-geometry programs, the coarse-graining a bounded observer
performs to perceive geometry is a candidate for exactly this angular
projection---but its strongest testable claim (dynamical, energy-responsive
curvature) is a null here (\S\ref{sec:neg}), so we offer it only as motivation; the
spectral contribution stands independent of it. Open problems: proving
...
(H1)--(H3) beyond the flat torus, where Corollary~\ref{cor:torus} settles them---%
the sharpest form being the uniform truncated-Jacobian bound of
Conjecture~\ref{conj:sigmamin}); extending
Corollary~\ref{thm:radial} to the truncated,
normalized radius actually used in practice; synthesizing clean manifolds above
$d\approx2$; and a dimension-adaptive angular basis that tunes
mode-count and diffusion time to counter the fidelity decay; and whether the
diagnostic transfers to learned semantic representations, for which a preliminary
probe is provided in the repository. Keep the angle,
drop the magnitude---\emph{when the magnitude is nuisance for the metric being
preserved}, which for graph geodesics holds provably for the full unnormalized
radius at $d\ge3$ (Corollary~\ref{thm:radial}) and empirically for the truncated,
normalized radius actually in use (\S\ref{sec:verify}):
the basis-level fact is the contribution; the physical interpretation is where we
leave the door open.

\paragraph{Reproducibility.} The pre-registration documents (\texttt{PREREG\_%
...
screen, the continuum and scaled-budget refinements, and the truncation/normalization
checks), the manifold-rule sets behind the $20$-point
dimension--fidelity trend (five rules---\texttt{R\_2D}, \texttt{R\_frac}, \texttt{evolved-2D},
...
scaled-budget refinement's builders (pending a vendored export).
...
uniform sampling or after the $\alpha{=}1$ reweighting of \S\ref{sec:density} that
...
\cite{calderlewicka2022} attaches to each discrete eigenvector $v_k$ a
...
$\{\tilde\varphi_k\}$ need not be exactly orthonormal, but their empirical Gram
matrix is $G=I+O(\varepsilon_n)$ (each off-diagonal is a discrete inner product of
two sup-norm-$\varepsilon_n$ perturbations of orthonormal continuum
eigenfunctions); the symmetric orthogonalization $G^{-1/2}=I+O(\varepsilon_n)$
therefore turns $\{\tilde\varphi_k\}$ into an exactly $L^2(\rho)$-orthonormal basis
$\{\varphi_k\}$ of the limiting eigenspace at cost $O(\varepsilon_n)$ in sup-norm,
and its per-multiplet action is the block-diagonal $Q_n\in O(m)$; each aligned
discrete eigenvector is then $(\epsilon_n/\sqrt m)$-close in sup-norm to its
partner. (Multiplicity is exactly why the alignment is block-orthogonal rather
...
\bibitem{alamgir2011} M.~Alamgir and U.~von~Luxburg. Phase transition in the
...
\bibitem{hein2007} M.~Hein, J.-Y.~Audibert, and U.~von~Luxburg. Graph
...
\bibitem{lipman2010biharmonic} Y.~Lipman, R.~M.~Rustamov, and
...
36(2):555--586, 2008.
\bibitem{vonluxburg2010getting} U.~von~Luxburg, A.~Radl, and M.~Hein.
