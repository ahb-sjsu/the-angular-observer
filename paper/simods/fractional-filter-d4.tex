...
\title{Past the critical dimension: the fractional filter for $d\ge4$}
...
question, monotonicity of the fractional Green kernel on two-point homogeneous spaces. This
note carries the entire chain up one level to the fractional filter and, using
...
spaces regardless of dimension. Notation follows Paper~I, the Green-kernel rank-limit note,
and the companion note ``green-rank-positivity''.
...
radial profile. Restating (companion note, ``green-rank-positivity'', Theorem ``Master
inequality''): for any strictly increasing $\Psi$ with remainder
...
$\phi_{\Psi_s}$ is the change-of-variables computation of the companion note with exponent
...
openness argument (companion reductions note) transfers with $\mu_k^{-1}\to\mu_k^{-s}$---its
...
\section{Imported facts and status}
\noindent\textbf{Imported.} (1) Weyl's law $\mu_k\asymp k^{2/d}$ and the uniform pointwise
Weyl law with remainder $O(L^{(d-1)/2})$ (H\"ormander). (2) Riesz-potential/heat-parametrix
structure of $\Delta^{-s}$: classical $\Psi$DO of order $-2s$, kernel singularity
$\gamma_{d,s}\dM^{2s-d}$ with universal leading constant (Seeley; Gilkey, \emph{Invariance
Theory}). (3) Subordination formula $\lambda^{-s}=\Gamma(s)^{-1}\int_0^\infty t^{s-1}e^{-\lambda t}dt$.
(4) Strict monotonicity in distance of the heat kernel on compact two-point homogeneous spaces,
via the parabolic maximum principle applied to $v=\partial_rp_t$ (proved in
Theorem~\ref{thm:twopoint}; classical on $S^1$ through the theta function). (5) The master
inequality and openness machinery (companion notes), filter-agnostic.

\medskip
\noindent\textbf{Status.} For every $d$ and $s>d/4$: the fractional angular ranking converges
uniformly in the mode count to the fractional Green-kernel ranking
(Theorem~\ref{thm:fraclimit}); its coefficient is $1$ on all rank-one symmetric spaces
(Theorem~\ref{thm:twopoint}, all $s>0$) and, for $s>(d-1)/2$, $>0$ on an explicit near-radial / $C^2$-open class
(Theorem~\ref{thm:fracnear}). \emph{Open:} fractional-Green-rank positivity for general
metrics past the near-radial class (the $d\ge4$ analogue of the standing conjecture), and an
effective lower bound on the admissible $s$ beyond $s>d/4$ if a sharper diagonal control is
wanted.
