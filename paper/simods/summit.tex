...
\]
The uniform pointwise Weyl law gives $S_\Lambda(x)=A_\Lambda(1+r_\Lambda(x))$
...
contradicting Aronszajn's strong unique continuation for smooth---indeed
Lipschitz---metrics).  Hence $\Phi_j(x,\cdot)\equiv0$ for such $x$, i.e.\
...
\textup{(ND)}---or $G_j(x)=0$ for a.e.\ $x$ in the $x$-projection of (the
relevant part of) $E$.  Under \textup{(ND)} the second alternative holds for
