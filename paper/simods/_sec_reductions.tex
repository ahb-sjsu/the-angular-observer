...
$P$ and $\widehat V_P$ for the graph value. Ties in $V$ are harmless: a tied pair
$\lvert V_P-V_Q\rvert=0\le2\bar\varepsilon_n$ already lies inside the flip event that Step~1
counts, provided both rankings use one fixed tie-breaking (midrank) convention; and the
empirical-to-population step, read in the midrank convention, converges because the limit
(Green) law is atomless (Lemma~\ref{lem:atomless}).
