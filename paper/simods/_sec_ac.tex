...
\noindent\textbf{Purpose.} The rank-transfer theorem (\S\ref{sec:reductions},
...
\]
$Y$ drawn from the sampling density. This Part does two things. First, it shows that
...
correct hypothesis, exactly as \S\ref{sec:reductions} anticipated (``bounded density is the
...
and their real-analytic deformations). Notation follows \S\ref{sec:reductions}; the cutoff
...
\begin{definition}[(AC$'$), modulus form]\label{def:acprime}
...
\]
jointly real-analytic in $(x,y)\in M\times M$ (the denominator is bounded below: by
H\"ormander's uniform local Weyl law the pointwise spectral function satisfies $e(\Lambda,x)>1$
for all $x$ once $\Lambda\ge\Lambda_1(M)$, so at every point some nonconstant mode $\le\Lambda$
is nonzero and $S_\Lambda(x)>0$). For each fixed $x$, $u_x$ is a
...
$\to0$). Both mitigations noted in \S\ref{sec:reductions} survive and are strengthened: (i)
...
the density scale is $\kappa_\Lambda\asymp A_\Lambda$ (\S\ref{sec:reductions}, Remark on the
