...
(I)$\le C\eta\,(Z^{g_0}(c t)-1)^{1/2}$ (standard parabolic perturbation; \S\ref{sec:frontier}).
Both envelopes are $\asymp t^{-d/4}$
...
\begin{remark}[Status: proof under revision per expert review]\label{rem:pert-status}
Expert review identifies two gaps in the proof as written, both fixable and being repaired:
(I) the naive Duhamel bound is \emph{not} integrable at $s\to0$
($\lVert(\Delta_g-\Delta_{g_0})p^{g_0}_s\rVert_2\asymp\eta\,s^{-1-d/4}$) and needs a half-time
split ($\int_0^{t/2}$ and $\int_{t/2}^t$ handled separately) or a direct parametrix
comparison; (II) the shell-projection difference must be controlled by the \emph{relative}
distance bound $\lvert d_g-d_{g_0}\rvert\le C\eta\,d_{g_0}$ (the Step-1 metric comparability of
\S\ref{sec:reductions}), not the absolute $\lVert d_g-d_{g_0}\rVert_\infty=O(\eta)$, which
leaves a divergent $t^{-1/2}$ from the Gaussian derivative. The conclusion
$V_{\min}(g)\le C(g_0)\eta^2$ is expected to survive; the repaired argument is to be supplied.
\end{remark}
