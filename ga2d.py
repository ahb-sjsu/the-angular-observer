...
def mutate(rule, rng):
    ...
    for _ in range(6):                       # try to produce a valid mutant
        L = [list(e) for e in lhs]; R = [list(e) for e in rhs]
        op = rng.integers(4)
        if op == 0:                          # retoken a random rhs slot
            e = R[rng.integers(len(R))]; e[rng.integers(3)] = str(rng.choice(toks))
            ...
            e = L[rng.integers(len(L))]; e[rng.integers(3)] = str(rng.choice(EXIST[:4]))
        Lt = [tuple(e) for e in L]; Rt = [tuple(e) for e in R]
        if validate3(Lt, Rt):
            return Lt, Rt
    return rule
...
def main():
    ...
    print(f"  {'gen':>3} | {'best_fit':>8} | best CLEAN (spread<=.3, arho>=.8): "
    ...
    with mp.Pool(WORKERS) as pool:
        for g in range(GENS):
            ...
            clean = [m for m in scored if m.get("valid") and m["spread"] <= 0.3
                     and m["angle_rho"] >= 0.8]
            clean.sort(key=lambda d: d["dim"], reverse=True)
            bc = clean[0] if clean else None
            if bc and (best_clean_overall is None
                       or bc["dim"] > best_clean_overall["dim"]):
                best_clean_overall = bc
            bc_s = (f"{bc['dim']:5.2f} {bc['angle_rho']:5.2f} {bc['spread']:6.2f}"
                    if bc else "   none")
            print(f"  {g:3d} | {scored[0]['fit']:8.3f} | {bc_s}")

            elite = [(m["lhs"], m["rhs"]) for m in scored[:POP // 2]]
            children = [mutate(elite[rng.integers(len(elite))], rng)
    ...
    if best_clean_overall:
        ...
        print(f"  dim={b['dim']:.2f} angle_rho={b['angle_rho']:.2f} "
              f"spread={b['spread']:.2f} N={b['N']}")
        print(f"  {b['lhs']} -> {b['rhs']}")
        print(f"  {'==> reached clean 2D+!' if b['dim'] >= 2.0 else '==> still under 2D; evolution confirms the ceiling.'}")
    else:
