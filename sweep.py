...
def main():
    ...
    print(f"sweep: draws={a.draws} workers={a.workers} "
          f"max_edges={MAX_EDGES} timeout={TIMEOUT_S}s", flush=True)
    results, done = [], 0
    with mp.Pool(a.workers) as pool:
        for m in pool.imap_unordered(worker, range(a.draws), chunksize=8):
            ...
            if done % 1000 == 0:
                ...
                print(f"  {done}/{a.draws} drawn | {len(results)} scored | "
                      f"best_score={best:.3f}", flush=True)
    ...
    for m in results[:20]:
        print(f"  {m['score']:6.3f} {m['angle_rho']:9.3f} {m['spread']:7.2f} "
              f"{m['dim']:5.2f} {m['N']:5d}   {m['lhs']} -> {m['rhs']}", flush=True)
