...
$0.39$--$0.65$ against $\approx0.10$ for the closed controls); one rule is,
at every growth stage and seed tested, an almost perfect \emph{interval}
(spectrum ratios $1,3.8,8.7,14.8$ against the $1,4,9,16$ template); two are
unambiguously two-dimensional bounded patches whose finer symmetry class is
less stable than their dimension; one is genuinely fractal ($d\approx1.4$);
and one is a $\approx3.4$-dimensional space matching no manifold template. Two findings are
...
paper's manifold library (arity-3, grown to the full 3000-edge budget with
budget-scaled generation cap, seed 1; seed stability in \S\ref{sec:seeds}).
...
\midrule
R\_2D    & 2000 & 0.28 & 1.05 (0.29) & 0.93 & 0.21 & 0.64 & interval (a curve) \\
...
R\_3D    & 2000 & 0.18 & 2.12 (0.34) & 0.67 & 0.57 & 0.50 & 2D, torus-leaning \\
...
\textbf{(ii) The recognizable cases are genuinely recognizable.} R\_2D is
identified as an \emph{interval} with near-exact spectrum
($1,3.8,8.7,14.8,24.0$ against $1,4,9,16,25$; signature distance $0.048$);
R\_3D is unambiguously two-dimensional---all 2D templates score far below
interval ($0.28$--$0.68$ vs $2.32$), flat-torus-leaning at this budget,
though \S\ref{sec:growth} shows the winning 2D template wanders with growth,
so ``2D'' is the claim and the finer symmetry class is not.
...
rules have the highest magnitude-$\rho$ ($0.29$, $0.47$): the radius stays
...
Each verdict above is one growth instance; rewrite rules are nondeterministic
in general, so verdicts must survive the rewriter seed. Re-running the full
battery at seeds $1$--$4$ (full 3000-edge budget):

\textbf{R\_2D: stable, 4/4 interval} ($d=1.05\pm0.01$,
angle-$\rho=0.93\pm0.00$)---the curve is a property of the rule.
\textbf{R\_frac: dimension rock-stable} ($d=1.39\pm0.00$ across seeds), with
the template tipping once (3/4 interval, 1/4 square patch): a fractal
filament web genuinely sits between the two templates.
\textbf{R\_3D: growth is deterministic}---all four seeds produce
byte-identical spectra ($d=2.12$, angle-$\rho=0.67$), so this rule evidently
admits a single applicable rewrite per step; seed replication is vacuous for
it and the growth axis of \S\ref{sec:growth} is the meaningful robustness
check. \textbf{evolved-2D: genuinely seed-dependent}: $d=1.87\pm0.14$ but
angle-$\rho$ ranges $0.38$--$0.89$ and the verdict scatters
(patch/torus/interval)---this rule does not reliably grow the same geometry,
and one seed grows a substrate whose angular fidelity collapses. \textbf{R\_SR:
4/4 non-manifold} ($d=3.39\pm0.11$): the winning template flips per seed with
uniformly weak scores ($0.25$--$0.55$), which is what ``matches none of
these'' should look like under replication. The meta-pattern of
\S\ref{sec:growth} repeats across seeds: dimension is the stable invariant
($\pm0.14$ at worst); the finer symmetry class is stable only where the
geometry actually is one of the model spaces.
...
a growing universe), at \emph{all four rewriter seeds per stage} ($60$
instances; Fig.~\ref{fig:growth}, where the band is the min--max over seeds).
Three growth behaviours appear, none the naive ``template crossing'' one might
expect:

\textbf{R\_2D is a curve, definitively.} The interval signature is pinned at
distance $0.02$--$0.05$ at every stage \emph{and every seed} ($20/20$
instances), with $d=1.05\pm0.01$ and angle-$\rho=0.93$--$0.95$ throughout;
the interior of the large substrate is as one-dimensional as its early
stages. The verdict of \S\ref{sec:rules} is a property of the rule.

\textbf{R\_3D: dimension locks first, shape does not---and the wandering is
deterministic.} All four seeds produce byte-identical spectra at \emph{every}
budget (the zero-width bands in Fig.~\ref{fig:growth}): this rule's growth is
seed-independent, so its template trajectory---square patch ($750$) $\to$
sphere ($1500$) $\to$ flat torus ($3000$) $\to$ sphere ($6000$) $\to$ square
patch ($12{,}000$)---is \emph{reproducible structure of the rule's growth},
not sampling noise. The measured dimension sits at $d\approx1.9$--$2.2$ from
the earliest stage while all three 2D templates score far below interval
($0.24$--$0.68$ vs $1.85$--$2.32$). The refined claim:
\emph{dimensionality is the robust emergent invariant; symmetry class is
not}---R\_3D is unambiguously a 2D geometry that is not (yet, or ever) any of
the clean model spaces. Its angular fidelity dips and recovers across the
sweep ($0.80\to0.59\to0.73$, identical at all seeds), consistent with the
fixed-budget observer effect of the companion paper's continuum section: a
fixed $m=20$ window resolves a shrinking fraction of a growing substrate.

\textbf{evolved-2D: seed sensitivity switches on at a growth scale.} Its
verdict is a stable square patch at every seed for $B\le1500$
(deterministic at $750$; angle-$\rho$ sd $0.01$ at $1500$), and then the
seeds \emph{diverge}: for $B\ge3000$ the across-seed angle-$\rho$ spread
jumps to $\pm0.21$ and the verdicts scatter (patch/torus/interval)---the
widening bands in Fig.~\ref{fig:growth}. The rule has a branching point:
early growth is canalized, later growth is contingent, and the recognizer
localizes the transition between $1500$ and $3000$ rewrite events.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{growth_multiseed.png}
\caption{Which manifold template fits, as the substrate grows---across all
four rewriter seeds (line = mean, band = min--max over seeds; lower = closer;
measured dimension annotated per stage). R\_2D (left): an interval at every
stage and seed. R\_3D (middle): \emph{zero-width bands}---growth is
deterministic, so the wandering of the winning 2D template is reproducible
structure. evolved-2D (right): the bands blow open after $B\approx1500$---%
seed sensitivity switches on at a specific growth scale.}
\label{fig:growth}
\end{figure}
...
the \emph{other way}, and the seed sweep (\S\ref{sec:seeds}, $20$ instances)
shows the effect is \emph{graded in density irregularity}: for the
low-irregularity rules ($\mathrm{CV}\le0.3$: R\_2D, R\_frac, R\_3D) the
$\alpha{=}1$ shift is $-0.001$ to $+0.011$---indistinguishable from zero---%
while for the high-irregularity rules it is consistently negative: R\_SR
($\mathrm{CV}\approx0.9$) loses $0.03$--$0.06$ at every seed, and evolved-2D
($\mathrm{CV}\approx1.4$) loses up to $0.04$ in three of four seeds. The
reading: on a graph with no latent manifold underneath, the density profile is
not noise over a ground truth---it \emph{is} part of the substrate's
structure, and the more structured the density, the more signal the
correction removes. This gives the density coordinate an unexpected second
life: for sampled geometry it is a nuisance parameter, for emergent geometry
it is data, and the sign of the $\alpha{=}1$ response is itself a diagnostic
for which situation one is in.

\section{Limitations and falsifiers}\label{sec:limits}
Seed replication is done on both axes (\S\ref{sec:seeds}: two rules fully
stable, one deterministic, one tipping between adjacent templates, one
genuinely seed-dependent; \S\ref{sec:growth}: all four seeds at every growth
stage, $60$ instances); hop-count geodesics; $n\le2000$ dense-eigensolver
scale; four
