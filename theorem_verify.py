...
def build_rewrite(max_edges):
    edges, nid = rewrite(_REWRITE_LHS, _REWRITE_RHS, [(0, 1), (0, 2), (1, 2)],
                         max_edges=max_edges, max_gen=800, seed=1)
...
def measure(A, m=M_MODES, n_anchor=250):
    ...
    return dict(N=n, deg=float(d.mean()), rho_rad=rho_rad, rho_ang=rho_ang,
                rho_full=rho_full, rho_R=rho_R, vl=vl, rho_rad_deg=rho_rad_deg)
...
def run_family(name, builder, specs):
    ...
    print(f"  {'N':>5} {'<deg>':>5} | {'(a)radius':>9} {'(b)ANGLE':>8} "
    ...
    for tN in specs:
        ...
        r = measure(A)
        rows.append(r)
        print(f"  {r['N']:5d} {r['deg']:5.1f} | {r['rho_rad']:9.3f} {r['rho_ang']:8.3f} "
              f"{r['rho_full']:10.3f} {r['rho_R']:10.3f} | {r['vl']:11.3f} {r['rho_rad_deg']:13.3f}")
    a = rows[0]; b = rows[-1]
    print(f"  -> ANGLE  {a['rho_ang']:.3f} -> {b['rho_ang']:.3f}  (flat?)"
...
def m_sweep(name, A):
    ...
    for m in (5, 10, 20, 40, 80):
        r = measure(A, m=m)
        print(f"  {m:4d} | {r['rho_ang']:6.3f} {r['rho_full']:8.3f} {r['rho_rad']:6.3f}")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 78)
    print("READING: (a) radius ~ 0  (b) ANGLE high & flat in N  (c) fullPhi & R decay in N")
